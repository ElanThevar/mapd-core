...
ExecutionResult RelAlgExecutor::executeWorkUnit(const RelAlgExecutor::WorkUnit& work_unit,
  ...
  if (!eo.just_explain && eo.find_push_down_candidates) {
    static constexpr float selectivity_threshold = 0.4;
    ...
    for (const auto& candidate : all_push_down_candidates) {
      if (getFilterSelectivity(candidate.filter_expressions, co, eo) < selectivity_threshold) {
...
float RelAlgExecutor::getFilterSelectivity(const std::vector<std::shared_ptr<Analyzer::Expr>>& filter_expressions,
                                           const CompilationOptions& co,
                                           const ExecutionOptions& eo) {
  ...
  constexpr size_t table_row_count_threshold = 1000;
  if (table_infos.front().info.getNumTuplesUpperBound() < table_row_count_threshold) {
    return 1.0;
  }
  try {
  ...
  const auto rows_total = std::max(table_infos.front().info.getNumTuplesUpperBound(), size_t(1));
  return static_cast<float>(rows_passing) / rows_total;
}
